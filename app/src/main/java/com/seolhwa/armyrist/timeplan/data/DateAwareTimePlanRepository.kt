...
class DateAwareTimePlanRepository(context: Context) {
    ...
    fun commit(value: DateAwareTimePlan): Boolean {
        ...
        if (DateTimePlanRules.validate(value).isNotEmpty()) return false
    ...
    fun importAsNew(source: DateAwareTimePlan): String? {
        ...
        if (DateTimePlanRules.validate(fresh).isNotEmpty()) return null
    ...
    fun replaceAll(restored: List<DateAwareTimePlan>): Boolean {
        if (restored.any { DateTimePlanRules.validate(it).isNotEmpty() }) return false
    ...
    fun migrateLegacy(legacy: RevisedTimePlan, baseDate: LocalDate): DateAwareTimePlan? {
        ...
        if (DateTimePlanRules.validate(candidate).isNotEmpty()) return null
    ...
    private fun load(): List<DateAwareTimePlan> {
        ...
        return runCatching {
            ...
            require(root.getInt("schemaVersion") == SCHEMA)
...
object DateAwareTimePlanJson {
    fun encode(p: DateAwareTimePlan): JSONObject = JSONObject()
        .put("schemaVersion", 3).put("id", p.id).put("title", p.title)
        .put("start", valueToJson(p.start.value))
        ...
        .put("end", valueToJson(p.end.value))
    ...
    fun decode(j: JSONObject): DateAwareTimePlan {
        require(j.getInt("schemaVersion") == 3)
        ...
        return DateAwareTimePlan(
            ...
            start=DateTimeAnchor(valueFromJson(j.getJSONObject("start"))),
            ...
            end=DateTimeAnchor(valueFromJson(j.getJSONObject("end"))),
    ...
    private fun eventToJson(e:DateTimeEvent)=JSONObject().put("id",e.id).put("kind",e.kind.name).put("order",e.order).put("name",e.name).put("timeSpec",specToJson(e.timeSpec)).put("note",e.note?:JSONObject.NULL)
    private fun eventFromJson(j:JSONObject)=DateTimeEvent(j.getString("id"),TimeEventKind.valueOf(j.getString("kind")),j.getInt("order"),j.getString("name"),specFromJson(j.getJSONObject("timeSpec")),if(j.isNull("note")) null else j.getString("note"))
    private fun linkToJson(l:DateTimeLink)=JSONObject().put("fromNodeId",l.fromNodeId).put("toNodeId",l.toNodeId).put("minutes",l.durationMinutes?:JSONObject.NULL).put("origin",l.origin.name).put("label",l.label?:JSONObject.NULL)
    private fun linkFromJson(j:JSONObject)=DateTimeLink(j.getString("fromNodeId"),j.getString("toNodeId"),if(j.isNull("minutes")) null else j.getLong("minutes"),ValueOrigin.valueOf(j.getString("origin")),if(j.isNull("label")) null else j.getString("label"))
}
